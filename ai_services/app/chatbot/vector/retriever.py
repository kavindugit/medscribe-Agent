...
def retrieve_chunks(query: str, case_id: str, user_id: str, top_k: int = 5) -> Dict[str, Any]:
    ...
    _ = get_qdrant_vectorstore(collection_name)
    ...
    if user_id:
        filter_conditions.append(
            qmodels.FieldCondition(
                key="user_id",
                match=qmodels.MatchValue(value=str(user_id))  # ensure string
            )
        )
    if case_id:
        filter_conditions.append(
            qmodels.FieldCondition(
                key="case_id",
                match=qmodels.MatchValue(value=str(case_id))  # ensure string
            )
        )
